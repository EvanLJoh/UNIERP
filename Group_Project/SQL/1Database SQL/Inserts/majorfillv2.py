...
for major_name, major_requirements in major_reqs.items():
    # Extract the credit requirements
    credits_required = sum([int(course[1]) for course in major_requirements])
    ...
    while sum([int(course[1]) for course in major_requirements]) < credits_required:
        ...
        if course not in major_requirements:
           major_requirements.append((course[0], course[2]))
